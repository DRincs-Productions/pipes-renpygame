    ...
    def check_connection_helper(self, mat: Matrix, images_resized: dict) -> list:
        ...
        for i, con in enumerate(water_connections):
            if con:
                if i == 0:
                    edges.append(
                        (
                            (self.node_up.pos[0], self.node_up.pos[1]),
                    ...
                    edges.append(
                        (
                            (self.pos[0], self.pos[1]),
                            (self.node_right.pos[0], self.node_right.pos[1]),
                        )
                    )
                elif i == 2:
                    edges.append(
                        (
                            (self.pos[0], self.pos[1]),
                            (self.node_down.pos[0], self.node_down.pos[1]),
                        )
                    )
                elif i == 3:
                    edges.append(
                        (
                            (self.node_left.pos[0], self.node_left.pos[1]),
                            (self.pos[0], self.pos[1]),
                        )
                    )
        if self.type < 5:
        ...
        if self.with_water:
            ...
            if connections[1] and not self.node_right.checked:
                edges.extend(
                    self.node_right.check_connection_helper(mat, images_resized)
                )
            if connections[2] and not self.node_down.checked:
                edges.extend(
                    self.node_down.check_connection_helper(mat, images_resized)
                )
            if connections[3] and not self.node_left.checked:
                edges.extend(
                    self.node_left.check_connection_helper(mat, images_resized)
                )
        self.image = image_getter(self.type, self.with_water, images_resized)
        return edges
...
def get_images() -> dict:
    ...
    if SYSTEM == "Windows":
        for st in start:
            for ed in end:
                aux_dict[st] = aux_dict.get(st, ()) + (
                    pygame.image.load("Images\\" + st + "_" + ed + ".png"),
                )
        start = ["One_Way", "Straight", "Two_Way", "Three_Way", "Four_Way"]
        for st in start:
            aux_dict[st + "_Source_Node"] = pygame.image.load(
                "Images\\" + st + "_Source_Node.png"
            )
...
def resize_images(side: int, images: dict):
    ...
    for st in start:
        aux_dict[st] = (
            pygame.transform.scale(images[st][0], image_size),
            pygame.transform.scale(images[st][1], image_size),
        )
    start = ["One_Way", "Straight", "Two_Way", "Three_Way", "Four_Way"]
    for st in start:
        aux_dict[st + "_Source_Node"] = pygame.transform.scale(
            images[st + "_Source_Node"], image_size
        )
    return aux_dict
...
def change_theme(curr: str, new: str, images: dict, image_color_themes: dict):
    ...
    for st in start:
        for i in range(2):
            for j in range(3):
                pygame.PixelArray(images[st][i]).replace(
                    translation[0][j], translation[1][j]
                )
    start = ["One_Way", "Straight", "Two_Way", "Three_Way", "Four_Way"]
    for st in start:
        for j in range(3):
            pygame.PixelArray(images[st + "_Source_Node"]).replace(
                translation[0][j], translation[1][j]
            )
...
def main():
        ...
        while loops_exist(matrix, images_resized):
            while everything_is_connected(matrix, images_resized):
                ...
                new_rect = pygame.Rect(
                    (SCREEN_SIZE[0] - loading_bar_rect["width"]) // 2,
                    SCREEN_SIZE[1] // 16 * 13,
                    l,
    ...
    colors = {
        "passive": pygame.Color((102, 102, 102)),
    ...
    image_color_themes = {
        "default": (
            (100, 180, 230),  # Light Blue (Water)
            (60, 60, 150),  # Dark blue (Center Node)
            (80, 160, 200),
        ),  # Receiver Node
        "red": ((170, 15, 15), (100, 5, 5), (170, 60, 60)),
        "green": ((15, 170, 15), (5, 90, 5), (70, 190, 70)),
        "yellow": ((200, 200, 20), (160, 160, 0), (240, 240, 70)),
        "white": ((220, 220, 220), (180, 180, 180), (255, 255, 255)),
        "black": ((30, 30, 30), (0, 0, 0), (55, 55, 55)),
    }

    # --- Language Constants --- #
    language = "english"
    english = {
        "OutOfRange": "[ERROR] The number you typed is out of range.",
    ...
    portuguese = {
        "OutOfRange": "[ERRO] O número que digitou está fora do intervalo.",
    ...
    while running:
        ...
        for ev in pygame.event.get():
                            ...
                            if input_check == "Clear":
                                if is_timer_back:
                                    ...
                                    if SCREEN_SIZE[0] <= h:
                                        grid_origin = (
                                            0,
                                            (h - SCREEN_SIZE[0]) // 2 + timer_up_space,
                                        )
                                    else:
                                        grid_origin = (
                                            (SCREEN_SIZE[0] - h) // 2,
                                ...
                                grid_origin = (
                                    grid_origin[0] + difference // 2,
                                    grid_origin[1] + difference // 2,
                                )
                                grid_back_rect_screen = pygame.Surface(
                                    (grid_size, grid_size)
                                )
                                grid_back_rect_screen.set_alpha(grid_back_alpha)
                                curr_back_color = colors["grid_back"]

                                images_resized = resize_images(
                                    images_side_length, images
                                )
                                ...
                                # Size Text
                                loading_size_surface = loading_size_font.render(
                                    languages[language]["size"] + str(ipt),
                                    antialias,
                                    loading_color,
                                )
                                screen.blit(
                                    loading_size_surface,
                                    (
                                        (
                                            SCREEN_SIZE[0]
                                            - loading_size_surface.get_width()
                                        )
                                        // 2,
                                        (
                                            SCREEN_SIZE[1]
                                ...
                                loading_surface = loading_font.render(
                                    loading_text, antialias, loading_color
                                )
                                screen.blit(
                                    loading_surface,
                                    (
                                        (SCREEN_SIZE[0] - loading_surface.get_width())
                                        // 2,
                                        (SCREEN_SIZE[1] - loading_surface.get_height())
                                        // 8
                                        * 3,
                                    ),
                                )
                    ...
                    if ev.type == pygame.MOUSEBUTTONDOWN:
                        if ev.button in (1, 3):
                            ...
                            if in_canvas_pixels(mouse_click, grid_origin, grid_size):
                                ...
                                mat_coords = (
                                    (mouse_click[0] - grid_origin[0])
                                    // images_side_length,
                                    (mouse_click[1] - grid_origin[1])
                                    // images_side_length,
                                )
                                curr_node = game_matrix[mat_coords[1]][mat_coords[0]]
                                edges, victory = curr_node.click(
                                    game_matrix, clockwise, images_resized
                                )
                                ...
                                if victory:
                                    ...
                                    victory_timer_text = time_formatter(
                                        perf_counter() - start_time
                                    )
                if ev.type == pygame.KEYDOWN:
                    if (
                        ev.key == pygame.K_RETURN and victory
                    ) or ev.key == pygame.K_ESCAPE:
                        curr_screen = "starting"
                        victory = False
                        victory_text = ""
                        victory_timer_text = ""
            ...
            if ev.type == pygame.VIDEORESIZE:
                ...
                textbox_size = (SCREEN_SIZE[0] // 8, SCREEN_SIZE[1] // 8)
                textbox_pos = (
                    SCREEN_SIZE[0] // 2 - textbox_size[0] // 2,
                    SCREEN_SIZE[1] // 2,
                )
        ...
        if curr_screen == "starting":
            ...
            screen.blit(
                title_surface,
                (
                    (SCREEN_SIZE[0] - title_surface.get_width()) // 2,
                    SCREEN_SIZE[1] // 8,
                ),
            )
            ...
            textbox_surface = textbox_font.render(
                languages[language]["size"] + textbox_text,
                antialias,
                textbox_text_color,
            )
            ...
            textbox_surface_pos = (
                textbox_rect.left + textbox_padding,
                textbox_rect.bottom - textbox_surface.get_height() - textbox_padding,
            )
            ...
            screen.blit(
                info_surface,
                (
                    (SCREEN_SIZE[0] - info_surface.get_width()) // 2,
            ...
            screen.blit(
                error_surface,
                (
                    (SCREEN_SIZE[0] - error_surface.get_width()) // 2,
            ...
            settings_hint_surface = settings_hint_font.render(
                settings_hint_text, antialias, settings_hint_color
            )
            screen.blit(
                settings_hint_surface,
                (
                    (SCREEN_SIZE[0] - settings_hint_surface.get_width()) // 2,
                    SCREEN_SIZE[1] // 4 * 3,
                ),
            )

            # Credits
            credits_surface = credits_font.render(
                credits_text, antialias, credits_color
            )
            screen.blit(
                credits_surface,
                (
                    (SCREEN_SIZE[0] - credits_surface.get_width()) // 2,
                    SCREEN_SIZE[1] // 8 * 7,
                ),
            )
            ...
            if is_timer_back:
                # Timer Behind Grid
                timer_surface = timer_font_background.render(
                    timer_text, antialias, timer_color
                )
                timer_surface.set_alpha(timer_alpha)
                screen.blit(
                    timer_surface,
                    (
                        (SCREEN_SIZE[0] - timer_surface.get_width()) // 2,
            ...
            if is_grid_on:
                for x in range(
                    grid_origin[0], grid_origin[0] + grid_size + 1, images_side_length
                ):
                    pygame.draw.line(
                        screen,
                        colors["grid_lines"],
                ...
                for y in range(
                    grid_origin[1], grid_origin[1] + grid_size + 1, images_side_length
                ):
                    pygame.draw.line(
                        screen,
                        colors["grid_lines"],
            ...
            for row in game_matrix:
                for item in row:
                    ...
                    image_pos = (
                        grid_origin[0] + item.pos[0] * images_side_length,
                        grid_origin[1] + item.pos[1] * images_side_length,
                    )
                    screen.blit(image, image_pos)

            # Victory Text
            victory_surface = victory_font.render(
                victory_text, antialias, victory_color
            )
            screen.blit(
                victory_surface,
                (
                    (SCREEN_SIZE[0] - victory_surface.get_width()) // 2,
                    (SCREEN_SIZE[1] - victory_surface.get_height()) // 2,
                ),
            )

            # Victory Timer Text
            victory_timer_surface = victory_timer_font.render(
                victory_timer_text, antialias, victory_timer_color
            )
            screen.blit(
                victory_timer_surface,
                (
                    (SCREEN_SIZE[0] - victory_timer_surface.get_width()) // 2,
            ...
            # Color Setting
            color_surface = settings_font.render(
                settings_color_text, antialias, settings_color
            )
            screen.blit(
                color_surface,
                (
                    l_cen - color_surface.get_width() // 2,
            ...
            pygame.draw.rect(
                screen,
                image_color_themes[curr_images_theme][0],
                pygame.Rect(
                    r_cen - settings_icons_size // 2,
                    SCREEN_SIZE[1] // options - settings_icons_size // 3,
                    settings_icons_size,
                    settings_icons_size // 3 * 2,
                ),
                border_radius=SCREEN_SIZE[1] // 80,
            )

            # Language Setting
            language_surface = settings_font.render(
                settings_language_text, antialias, settings_color
            )
            screen.blit(
                language_surface,
                (
                    l_cen - language_surface.get_width() // 2,
            ...
            screen.blit(
                ...
                (
                    r_cen - settings_icons_size // 2,
                    SCREEN_SIZE[1] // options * 2 - settings_icons_size // 2,
                ),
            )

            # Timer Setting
            settings_timer_surface = settings_font.render(
                settings_timer_text, antialias, settings_color
            )
            screen.blit(
                settings_timer_surface,
                (
                    l_cen - settings_timer_surface.get_width() // 2,
                    SCREEN_SIZE[1] // options * 3
                    - settings_timer_surface.get_height() // 2,
                ),
            )
            ...
            screen.blit(
                ...
                (
                    r_cen - settings_icons_size // 2,
                    SCREEN_SIZE[1] // options * 3 - settings_icons_size // 2,
                ),
            )

            # Grid Setting
            settings_grid_surface = settings_font.render(
                settings_grid_text, antialias, settings_color
            )
            screen.blit(
                settings_grid_surface,
                (
                    l_cen - settings_grid_surface.get_width() // 2,
                    SCREEN_SIZE[1] // options * 4
                    - settings_grid_surface.get_height() // 2,
                ),
            )

            grid_icon_pos = (
                r_cen - settings_icons_size // 2,
                SCREEN_SIZE[1] // options * 4 - settings_icons_size // 2,
            )
            pygame.draw.rect(
                screen,
                colors["grid_back_solid"],
                pygame.Rect(grid_icon_pos, (settings_icons_size, settings_icons_size)),
            )
            if is_grid_on:
                grid_icon_end = (
                    grid_icon_pos[0] + settings_icons_size,
                    grid_icon_pos[1] + settings_icons_size,
                )
                ...
                for x in range(grid_icon_pos[0], grid_icon_end[0] + 1, grid_icon_step):
                    pygame.draw.line(
                        ...
                        colors["grid_lines"],
                        (x, grid_icon_pos[1]),
                        (x, grid_icon_pos[1] + settings_icons_size),
                    )
                for y in range(grid_icon_pos[1], grid_icon_end[1] + 1, grid_icon_step):
                    pygame.draw.line(
                        ...
                        colors["grid_lines"],
                        (grid_icon_pos[0], y),
                        (grid_icon_pos[0] + settings_icons_size, y),
                    )
